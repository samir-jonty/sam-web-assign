...
class OwlCarousel2Item {
  ...
  public function __construct(array $item_array) {
    $this->setType($item_array['type'] ?? NULL);
    $this->setFileId($item_array['file_id'] ?? NULL);
    $this->setEntityId($item_array['entity_id'] ?? NULL);
    $this->setVideoUrl($item_array['video_url'] ?? NULL);
    $this->setViewId($item_array['view_id'] ?? NULL);
    $this->setWeight($item_array['weight'] ?? NULL);
    $this->setViewMode($item_array['view_mode'] ?? NULL);
    $this->setImageStyle($item_array['image_style'] ?? NULL);
    $this->setDisplayNodeTitle($item_array['display_node_title'] ?? FALSE);
    $this->setItemLabel($item_array['item_label'] ?? NULL);
    $this->setItemLabelType($item_array['item_label_type'] ?? 'content_title');
    $this->setContentOverImage($item_array['content_over_image'] ?? FALSE);
    $this->setContentVerticalPosition($item_array['content_vertical_position'] ?? '');
    $this->setContentHorizontalPosition($item_array['content_horizontal_position'] ?? '');
    $this->setContentPositionUnit($item_array['content_position_unit'] ?? '');
    $this->setContentPositionTop($item_array['content_position_top'] ?? '');
    $this->setContentPositionBottom($item_array['content_position_bottom'] ?? '');
    $this->setContentPositionLeft($item_array['content_position_left'] ?? '');
    $this->setContentPositionRight($item_array['content_position_right'] ?? '');
    $this->setTitleColor($item_array['title_color'] ?? '');
    $this->setContentColor($item_array['content_color'] ?? '');
    $this->setBackgroundColor($item_array['background_color'] ?? '');
    $this->setTextToDisplay($item_array['text_to_display'] ?? '');
    $this->setCustomText($item_array['custom_text'] ?? '');
    $this->setYoutubeControls($item_array['youtube_controls'] ?? FALSE);
    $this->setYoutubeShowinfo($item_array['youtube_showinfo'] ?? FALSE);
    $this->setYoutubeRel($item_array['youtube_rel'] ?? FALSE);
    $this->setYoutubeLoop($item_array['youtube_loop'] ?? FALSE);
    if (isset($item_array['navigation_image_id'])) {
      $this->setNavigationImageId(is_array($item_array['navigation_image_id']) ? ($item_array['navigation_image_id'][0] ?? $item_array['navigation_image_id']) : $item_array['navigation_image_id']);
    }
    else {
      $this->setNavigationImageId(NULL);
    }

    $id = $item_array['id'] ?? NULL;
