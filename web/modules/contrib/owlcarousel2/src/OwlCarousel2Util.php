...
class OwlCarousel2Util {
  ...
  private static function prepareImageCarousel(array $item, OwlCarousel2 $carousel) {
    ...
    if ($node) {
      ...
      $node_render_array += $view_builder->view($node, $item['view_mode']);
  ...
  public static function changeFile($file_id, OwlCarousel2 $carousel, $previous_file_id) {
    ...
    if ($file instanceof File) {
      $file_usage->add($file, 'owlcarousel2', $carousel->getEntityTypeId(), $carousel->id());
      $file->setPermanent();
      $file->save();
    }
    ...
    if ($previous_file_id) {
      ...
      if ($previous_file instanceof File) {
        $file_usage->delete($previous_file, 'owlcarousel2', $carousel->getEntityTypeId(), $carousel->id());
        // Delete file if it's not being used anywhere else.
        $usage = $file_usage->listUsage($previous_file);
        if (count($usage) == 0) {
